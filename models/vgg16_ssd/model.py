...
class VGGBase(nn.Module):
    ...
    def forward(self, image):
        ...
        """
        out = self.relu_1(self.conv1_1(image))  # (N, 64, 300, 300)
        out = self.relu_2(self.conv1_2(out))  # (N, 64, 300, 300)
        out = self.pool1(out)  # (N, 64, 150, 150)

        out = self.relu_3(self.conv2_1(out))  # (N, 128, 150, 150)
        out = self.relu_4(self.conv2_2(out))  # (N, 128, 150, 150)
        out = self.pool2(out)  # (N, 128, 75, 75)

        out = self.relu_5(self.conv3_1(out))  # (N, 256, 75, 75)
        out = self.relu_6(self.conv3_2(out))  # (N, 256, 75, 75)
        out = self.relu_7(self.conv3_3(out))  # (N, 256, 75, 75)
        out = self.pool3(
            out
        )  # (N, 256, 38, 38), it would have been 37 if not for ceil_mode = True

        out = self.relu_8(self.conv4_1(out))  # (N, 512, 38, 38)
        out = self.relu_9(self.conv4_2(out))  # (N, 512, 38, 38)
        out = self.relu_10(self.conv4_3(out))  # (N, 512, 38, 38)
        conv4_3_feats = out  # (N, 512, 38, 38)
        out = self.pool4(out)  # (N, 512, 19, 19)

        out = self.relu_11(self.conv5_1(out))  # (N, 512, 19, 19)
        out = self.relu_12(self.conv5_2(out))  # (N, 512, 19, 19)
        out = self.relu_13(self.conv5_3(out))  # (N, 512, 19, 19)
        out = self.pool5(out)  # (N, 512, 19, 19), pool5 does not reduce dimensions

        out = self.relu_14(self.conv6(out))  # (N, 1024, 19, 19)

        conv7_feats = self.relu_15(self.conv7(out))  # (N, 1024, 19, 19)
...
class AuxiliaryConvolutions(nn.Module):
    ...
    def forward(self, conv7_feats):
        ...
        """
        out = self.relu_1(self.conv8_1(conv7_feats))  # (N, 256, 19, 19)
        out = self.relu_2(self.conv8_2(out))  # (N, 512, 10, 10)
        conv8_2_feats = out  # (N, 512, 10, 10)

        out = self.relu_3(self.conv9_1(out))  # (N, 128, 10, 10)
        out = self.relu_4(self.conv9_2(out))  # (N, 256, 5, 5)
        conv9_2_feats = out  # (N, 256, 5, 5)

        out = self.relu_5(self.conv10_1(out))  # (N, 128, 5, 5)
        out = self.relu_6(self.conv10_2(out))  # (N, 256, 3, 3)
        conv10_2_feats = out  # (N, 256, 3, 3)

        out = self.relu_7(self.conv11_1(out))  # (N, 128, 3, 3)
        conv11_2_feats = self.relu_8(self.conv11_2(out))  # (N, 256, 1, 1)
...
class SSD300(nn.Module):
    ...
    def create_prior_boxes(self):
        ...
        prior_boxes.clamp_(
            0, 1
        )  # (8732, 4); this line has no effect; see Remarks section in tutorial
...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, priors_cxcy, threshold=0.5, neg_pos_ratio=3, alpha=1.0):
        ...
        self.smooth_l1 = (
            nn.L1Loss()
        )  # *smooth* L1 loss in the paper; see Remarks section in the tutorial
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        for i in range(batch_size):
            ...
            if n_objects == 0:
                true_classes[i] = torch.zeros(8732, device=device)
                continue
            overlap = find_jaccard_overlap(
        ...
        if pos_sum == 0:
            ...
            ) / pos_sum.float()  # (), scalar

        # TOTAL LOSS
        # print(f'{conf_loss=} {loc_loss=}')
        # if conf_loss.isnan() or conf_loss.isnan():
        #    print('NAN DETECTED')
        #    print(conf_loss_hard_neg)
        #    print(conf_loss_pos)
        #    exit(1)
