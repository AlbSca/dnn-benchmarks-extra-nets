...
def extract_files(data_path, save_path, sets):
    ...
    for j, set_name in enumerate(sets):
        # get set dir
        set_path = os.path.join(data_path, set_name, set_name)
        set_path_annot = os.path.join(
            data_path, "..", "annotations", "annotations", set_name
        )
        ...
        print(
            "\n> Extracting images + annotations from set: {} ({}/{})".format(
                set_name, j + 1, len(sets)
            )
        )
        ...
        for i, video in enumerate(fnames):
            ...
            print(
                ">> Processing video: {}/{} ({}/{})".format(
                    set_name, video_name, i + 1, len(fnames)
                )
            )
...
def extract_data(data_path, save_path, sets=None):
    ...
    """
    assert os.path.exists(data_path), "Must provide a valid data path: {}".format(
        data_path
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Caltech Pedestrian Dataset extractor Options."
    )
    parser.add_argument(
        "-data_path", default="", type=str, help="Dataset directory path."
    )
    parser.add_argument(
        "-save_path", default="", type=str, help="Store the extracted data into a dir."
    )
    ...
    assert any(
        data_path
    ), "Please insert a valid data path using the -data_path input arg."
