...
def calculate_mAP(
    ...
    print(
        f"True Images = {len(det_boxes)} True Boxes = {len(det_labels)} True Labels = {len(det_scores)}"
    )
    class_map = {k: v + 1 for v, k in enumerate(class_names)}
    ...
    print(
        f"True Images = {true_images.size()} True Boxes = {true_boxes.size()} True Labels = {true_labels.size()}"
    )
    ...
    average_precisions = {
        rev_class_map[c + 1]: v for c, v in enumerate(average_precisions.tolist())
    }
